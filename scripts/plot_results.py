...
santander_validation = pd.read_csv('data/santander_validation.csv') 
...
with open('results/res_qq_start/y_validation.pkl', 'rb') as f:
    y_validation = pickle.load(f)

with open('results/res_qq_start/ks_validation.pkl', 'rb') as f:
    ks_validation = pickle.load(f)

with open('results/res_qq_start/cvm_validation.pkl', 'rb') as f:
    cvm_validation = pickle.load(f)
...
# Define empty lists
y_train_tot = {}; y_validation_tot = {}; y_test_tot = {}
ks_train_tot = {}; ks_validation_tot = {}; ks_test_tot = {}
cvm_train_tot = {}; cvm_validation_tot = {}; cvm_test_tot = {}
for model in y_train:
    y_train_tot[model] = []; ks_train_tot[model] = []; cvm_train_tot[model] = []
    y_validation_tot[model] = []; ks_validation_tot[model] = []; cvm_validation_tot[model] = []
...
for node in y_validation['poisson']:
    for model in y_validation:
        y_validation_tot[model] += list(y_validation[model][node])
        ks_validation_tot[model] += [ks_validation[model][node].statistic]
        cvm_validation_tot[model] += [cvm_validation[model][node]]
...
ks_test_array = np.zeros((len(ks_test_tot['poisson']), 6))
for k, model in enumerate(ks_test_tot):
    ks_test_array[:,k] = ks_test_tot[model]

# Calculate the minimum of each row of ks_validation_array, and use these values to select the corresponding indices for each row of ks_test_array 
ks_test_array_min = ks_test_array[np.arange(len(ks_test_array)), np.argmin(ks_validation_array, axis=1)]
...
ks_train_combined = [ks_train_tot['poisson'], ks_train_tot['mep'], ks_train_tot['sep'], ks_train_tot['gbmep_start'], ks_train_tot['smep'], ks_train_tot['gbmep']]
# Create labels for the boxplots
labels = ['Poisson', 'MEP', 'SEP', 'GB-MEP (start times only)', 'SMEP', 'GB-MEP']
# Create a figure and axis
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(ks_train_combined, labels=labels)
...
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(cvm_train_combined, labels=labels)
# Set axis labels and a title
ax.set_xlabel('Groups')
ax.set_ylabel('Values')
ax.set_title('Side-by-Side Boxplots')
...
for node in y_validation['poisson']:
    ax[0,0].plot(x, y_validation['poisson'][node], c='lightgray', linewidth=.5) 
    ax[0,1].plot(x, y_validation['mep'][node], c='lightgray', linewidth=.5)
    ax[0,2].plot(x, y_validation['sep'][node], c='lightgray', linewidth=.5) 
    ax[1,0].plot(x, y_validation['smep'][node], c='lightgray', linewidth=.5)
    ax[1,1].plot(x, y_validation['gbmep_start'][node], c='lightgray', linewidth=.5)
    ax[1,2].plot(x, y_validation['gbmep'][node], c='lightgray', linewidth=.5)

for u in range(2):
    for v in range(3):
        ax[u,v].plot(x,x, c='black', ls='dotted', linewidth=1)

ax[0,0].plot(x, np.percentile(a=y_validation_tot['poisson'], q=x*100), linewidth=1, c='black')
ax[0,1].plot(x, np.percentile(a=y_validation_tot['mep'], q=x*100), linewidth=1, c='black')
ax[0,2].plot(x, np.percentile(a=y_validation_tot['sep'], q=x*100), linewidth=1, c='black')
ax[1,0].plot(x, np.percentile(a=y_validation_tot['smep'], q=x*100), linewidth=1, c='black')
ax[1,1].plot(x, np.percentile(a=y_validation_tot['gbmep_start'], q=x*100), linewidth=1, c='black')
ax[1,2].plot(x, np.percentile(a=y_validation_tot['gbmep'], q=x*100), linewidth=1, c='black')
...
# Combine the data into a list for plotting
ks_validation_combined = [ks_validation_tot['poisson'], ks_validation_tot['mep'], ks_validation_tot['sep'], ks_validation_tot['gbmep_start'], ks_validation_tot['smep'], ks_validation_tot['gbmep']]
# Create labels for the boxplots
labels = ['Poisson', 'MEP', 'SEP', 'GB-MEP (start times only)', 'SMEP', 'GB-MEP']
# Create a figure and axis
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(ks_validation_combined, labels=labels)
# Set axis labels and a title
ax.set_xlabel('Groups')
ax.set_ylabel('Values')
ax.set_title('Side-by-Side Boxplots')
...
# Combine the data into a list for plotting
cvm_validation_combined = [cvm_validation_tot['poisson'], cvm_validation_tot['mep'], cvm_validation_tot['sep'], cvm_validation_tot['gbmep_start'], cvm_validation_tot['smep'], cvm_validation_tot['gbmep']]
# Create labels for the boxplots
labels = ['Poisson', 'MEP', 'SEP', 'GB-MEP (start times only)', 'SMEP', 'GB-MEP']
# Create a figure and axis
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(cvm_validation_combined, labels=labels)
# Set axis labels and a title
ax.set_xlabel('Groups')
ax.set_ylabel('Values')
ax.set_title('Side-by-Side Boxplots')
...
# Combine the data into a list for plotting
ks_test_combined = [ks_test_tot['poisson'], ks_test_tot['mep'], ks_test_tot['sep'], ks_test_tot['gbmep_start'], ks_test_tot['smep'], ks_test_tot['gbmep'], ks_test_array_min]
# Create labels for the boxplots
labels = ['Poisson', 'MEP', 'SEP', 'GB-MEP (start times only)', 'SMEP', 'GB-MEP', 'Selected']
# Create a figure and axis
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(ks_test_combined, labels=labels)
# Set axis labels and a title
ax.set_xlabel('Groups')
ax.set_ylabel('Values')
ax.set_title('Side-by-Side Boxplots')
...
# Combine the data into a list for plotting
cvm_test_combined = [cvm_test_tot['poisson'], cvm_test_tot['mep'], cvm_test_tot['sep'], cvm_test_tot['gbmep_start'], cvm_test_tot['smep'], cvm_test_tot['gbmep']]
# Create labels for the boxplots
labels = ['Poisson', 'MEP', 'SEP', 'GB-MEP (start times only)', 'SMEP', 'GB-MEP']
# Create a figure and axis
fig, ax = plt.subplots(figsize=(8, 6))
# Create side-by-side boxplots
ax.boxplot(cvm_test_combined, labels=labels)
# Set axis labels and a title
ax.set_xlabel('Groups')
ax.set_ylabel('Values')
ax.set_title('Side-by-Side Boxplots')
