...
for file in glob.glob('results/res_gbmep_shared/*.pkl'):
    with open(file, 'rb') as f:
        res_gbmep_shared.update(pickle.load(f))
...
y_gbmep_start_shared_train = {}; y_gbmep_start_shared_test = {}; y_gbmep_shared_train = {}; y_gbmep_shared_test = {}; y_gbmep_start_train = {}; y_gbmep_start_test = {}; y_gbmep_train = {}; y_gbmep_test = {}

## Intialise dictionaries for KS scores
ks_poisson_train = {}; ks_mep_train = {}; ks_sep_train = {}; ks_smep_train = {}; ks_gbmep_start_shared_train = {}; ks_gbmep_shared_train = {}; ks_gbmep_start_train = {}; ks_gbmep_train = {}
ks_poisson_test = {}; ks_mep_test = {}; ks_sep_test = {}; ks_smep_test = {}; ks_gbmep_start_shared_test = {}; ks_gbmep_shared_test = {}; ks_gbmep_start_test = {}; ks_gbmep_test = {}

## Intialise dictionaries for Cramér-von Mises scores
cvm_poisson_train = {}; cvm_mep_train = {}; cvm_sep_train = {}; cvm_smep_train = {}; cvm_gbmep_start_shared_train = {}; cvm_gbmep_shared_train = {}; cvm_gbmep_start_train = {}; cvm_gbmep_train = {}
cvm_poisson_test = {}; cvm_mep_test = {}; cvm_sep_test = {}; cvm_smep_test = {}; cvm_gbmep_start_shared_test = {}; cvm_gbmep_shared_test = {}; cvm_gbmep_start_test = {}; cvm_gbmep_test = {}
...
for index in G.nodes:
    ...
    p_gbmep_start_shared_train, p_gbmep_start_shared_test = G.pvals_gbmep_start_shared(params=pp, node_index=index, subset_nodes=res_gbmep_start_shared[index].subset_nodes, start_times=start_times, test_split=True)
    # p-values for GB-MEP
    pp = gb_mep.transform_parameters(res_gbmep_shared[index].x)
    p_gbmep_shared_train, p_gbmep_shared_test = G.pvals_gbmep_start_self_shared(params=pp, node_index=index, subset_nodes=res_gbmep_shared[index].subset_nodes, start_times=start_times, end_times=end_times, test_split=True) 
    # p-values for GB-MEP with start times only
    ...
    y_gbmep_start_shared_train[index] = np.percentile(a=p_gbmep_start_shared_train, q=x*100)
    y_gbmep_shared_train[index] = np.percentile(a=p_gbmep_shared_train, q=x*100)
    ...
    if len(p_poisson_test) > 0:
        ...
        y_gbmep_start_shared_test[index] = np.percentile(a=p_gbmep_start_shared_test, q=x*100)
        y_gbmep_shared_test[index] = np.percentile(a=p_gbmep_shared_test, q=x*100)
    ...
    ks_gbmep_start_shared_train[index] = stats.kstest(p_gbmep_start_shared_train, stats.uniform.cdf)
    ks_gbmep_shared_train[index] = stats.kstest(p_gbmep_shared_train, stats.uniform.cdf)
    ...
    if len(p_poisson_test) > 0:
        ...
        ks_gbmep_start_shared_test[index] = stats.kstest(p_gbmep_start_shared_test, stats.uniform.cdf)
        ks_gbmep_shared_test[index] = stats.kstest(p_gbmep_shared_test, stats.uniform.cdf)
    ...
    cvm_gbmep_start_shared_train[index] = np.sqrt(stats.cramervonmises(rvs=p_gbmep_start_shared_train, cdf=stats.uniform.cdf).statistic / G.N[index])
    cvm_gbmep_shared_train[index] = np.sqrt(stats.cramervonmises(rvs=p_gbmep_shared_train, cdf=stats.uniform.cdf).statistic / G.N[index])
    ...
    if len(p_poisson_test) > 0:
        ...
        cvm_gbmep_start_shared_test[index] = np.sqrt(stats.cramervonmises(rvs=p_gbmep_start_shared_test, cdf=stats.uniform.cdf).statistic / G.N[index])
        cvm_gbmep_shared_test[index] = np.sqrt(stats.cramervonmises(rvs=p_gbmep_shared_test, cdf=stats.uniform.cdf).statistic / G.N[index])
...
y_train['gbmep_start_shared'] = y_gbmep_start_shared_train; y_train['gbmep_shared'] = y_gbmep_shared_train; y_train['gbmep_start'] = y_gbmep_start_train; y_train['gbmep'] = y_gbmep_train
ks_train['poisson'] = ks_poisson_train; ks_train['mep'] = ks_mep_train; ks_train['sep'] = ks_sep_train; ks_train['smep'] = ks_smep_train
ks_train['gbmep_start_shared'] = ks_gbmep_start_shared_train; ks_train['gbmep_shared'] = ks_gbmep_shared_train; ks_train['gbmep_start'] = ks_gbmep_start_train; ks_train['gbmep'] = ks_gbmep_train
...
cvm_train['gbmep_start_shared'] = cvm_gbmep_start_shared_train; cvm_train['gbmep_shared'] = cvm_gbmep_shared_train; cvm_train['gbmep_start'] = cvm_gbmep_start_train; cvm_train['gbmep'] = cvm_gbmep_train
y_test['poisson'] = y_poisson_test; y_test['mep'] = y_mep_test; y_test['sep'] = y_sep_test; y_test['smep'] = y_smep_test
y_test['gbmep_start_shared'] = y_gbmep_start_shared_test; y_test['gbmep_shared'] = y_gbmep_shared_test; y_test['gbmep_start'] = y_gbmep_start_test; y_test['gbmep'] = y_gbmep_test
ks_test['poisson'] = ks_poisson_test; ks_test['mep'] = ks_mep_test; ks_test['sep'] = ks_sep_test; ks_test['smep'] = ks_smep_test
ks_test['gbmep_start_shared'] = ks_gbmep_start_shared_test; ks_test['gbmep_shared'] = ks_gbmep_shared_test; ks_test['gbmep_start'] = ks_gbmep_start_test; ks_test['gbmep'] = ks_gbmep_test
cvm_test['poisson'] = cvm_poisson_test; cvm_test['mep'] = cvm_mep_test; cvm_test['sep'] = cvm_sep_test; cvm_test['smep'] = cvm_smep_test
cvm_test['gbmep_start_shared'] = cvm_gbmep_start_shared_test; cvm_test['gbmep_shared'] = cvm_gbmep_shared_test; cvm_test['gbmep_start'] = cvm_gbmep_start_test; cvm_test['gbmep'] = cvm_gbmep_test
