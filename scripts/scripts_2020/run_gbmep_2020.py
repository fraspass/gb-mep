...
if not os.path.exists('results_2020/res_gbmep'):
   os.makedirs('results_2020/res_gbmep')
...
with open('results_2020/res_smep.pkl', 'rb') as f:
    res_smep = pickle.load(f)

## Obtain results via model fitting
start_vals = gb_mep.insert_in_dictionary(d=res_smep, val=-1, pos=3)
res_gbmep = G.fit(x0=start_vals, subset_nodes=nodes_subset, start_times=True, end_times=True, distance_start=True, distance_end=False, thresh=.5, min_nodes=3, shared_parameters=True)

## Save the results
with open('results_2020/res_gbmep/res_gbmep_' + str(suffix) + '.pkl', 'wb') as f:
